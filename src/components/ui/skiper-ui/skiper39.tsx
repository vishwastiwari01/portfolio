...
const CrowdCanvas = ({ src, rows = 15, cols = 7 }: CrowdCanvasProps) => {
  ...
  useEffect(() => {
    ...
    const createPeep = ({ image, rect }: { image: HTMLImageElement; rect: number[] }): Peep => {
      const peep: Peep = {
        image, rect: [], width: 0, height: 0, drawArgs: [], x: 0, y: 0,
        anchorY: 0, scaleX: 1, walk: null,
        setRect: (rect: number[]) => {
    ...
    const resize = () => {
      ...
      canvas.width = stage.width * devicePixelRatio; canvas.height = stage.height * devicePixelRatio;
